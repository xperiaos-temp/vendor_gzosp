...
function setup_vendor() {
    ...
    export XPERIA_ROOT="$3"
    if [ ! -d "$XPERIA_ROOT" ]; then
        echo "\$XPERIA_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$XPERIA_ROOT/$OUTDIR" ]; then
        mkdir -p "$XPERIA_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$XPERIA_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$XPERIA_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$XPERIA_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$XPERIA_ROOT"/vendor/xperia/build/tools/smali/baksmali.jar
        export SMALIJAR="$XPERIA_ROOT"/vendor/xperia/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$XPERIA_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$XPERIA_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$XPERIA_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        DUMPDIR="$XPERIA_ROOT"/system_dump
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$XPERIA_ROOT"/vendor/xperia/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$XPERIA_ROOT"/"$OUTDIR"/radio
